...
/* ==== 距离动作制动提前量 ====================================================
   原实现是"走够了才开始刹车"，刹车过程本身走过的距离全部变成净超调，而且
   速度越快超调越大。现在改成提前判停：

       提前量 mm = 当前车速(mm/s) × CAR_BRAKE_LEAD_MS / 1000

   用**实测车速**而不是固定 mm 值，物理上才对 —— 制动距离近似正比于初速度，
   所以写成一个"等效制动时间"，高低速自动缩放，换速度不用重新标定。

   下面的 90 是**估计值，不是实测结果**。标定方法：
     1. 先置 0（关闭提前量），跑 Car_ForwardDistance(70U, 1000U)，
        量车头实际停下的位置，得超调量 over_mm；
     2. 看串口 [DIST] 最后一行打印的 speed=xxx mm/s，
        令 CAR_BRAKE_LEAD_MS = over_mm × 1000 / speed；
     3. 复跑验证。换速度不必重标，但换地面/轮胎要复查。

   置 0 即完全恢复改造前的行为，便于对比。
   ============================================================================ */
#define CAR_BRAKE_LEAD_MS        50U

/* 提前量上限占目标距离的比例（百分之几）。防止短距离动作被提前量吃掉：
   目标 100mm 而提前量算出 120mm 时会立刻判停、车根本没动。
   超过该比例就按该比例截断。 */
#define CAR_BRAKE_LEAD_MAX_PCT   40U
