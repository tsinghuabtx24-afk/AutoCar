/**
  ...
  * @brief   HC-SR04 超声波测距
  ******************************************************************************
  * @note    测距为**中断式非阻塞**：发 TRIG 后立即返回，ECHO 双边沿各触发一次
  *          EXTI，在中断里用 DWT 打时间戳，主循环只查状态。残留阻塞只有 TRIG
  *          那 10us 脉冲（DWT 忙等），HC-SR04 时序要求，无法省掉。
  *
  *          不用定时器输入捕获，因为 **ECHO 接在 PF12，F103 没有任何定时器通道
  *          映射到 GPIOF**，改用输入捕获得先改硬件接线。代价是中断响应有几百 ns
  *          抖动（EXTI15_10 与 IR_IN 共享向量时排队）；1mm 对应 5.8us 回波，
  *          不影响毫米级精度。
  ******************************************************************************
  */
...
/* 阻塞接口，仅标定用，正式主循环不要调。内部就是"Start + 循环 Step 到终态"，
   与非阻塞路径共享同一份换算逻辑，标定值不会两处不同步。 */
