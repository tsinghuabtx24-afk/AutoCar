...
| ~~5~~ | **已完成** 循迹改差速连续修正（§8.1） | 直线不摆动 → 缓弯不切内道 → 再提速 |
| ~~6~~ | **已完成** 超声波非阻塞测距（§8 末，**改用 EXTI 而非输入捕获**） | 消除最后一处忙等 |
| ~~7~~ | **已完成** 修 §2.4 缺陷；遥控帧队列（§8.2） | 1 号库后仍能接受新事件；连发事件不丢 |
...
- 1、2 档：`Car_DriveSpeed(base ∓ level×step, base ± level×step)`，车不停。
- 3 档（极左/极右）：切原地转。**没有用 `Car_StartRotateAngle()`**——那是
  "转到指定角度就停"，而循迹要的是"转到压回线就停"。后者是传感器闭环，比按
  角度开环准，也不依赖 `CAR_ROT_COMP_*` 滑移补偿表。用瞬时接口 + 每周期判
  退出，同样非阻塞。这是对 §8.1 原描述的一处修正。
- 全白持续 `LINE_LOST_COUNT` 个周期判丢线，制动并投 `EVENT_FAULT`。原实现的
  `LINE_STOP_ON_UNKNOWN` 只覆盖单次未定义图案，车会带着上次轮速继续冲。

两个容易踩的坑，已用机制挡住：

- **死区约束**。`car` 的死区上界是 `CAR_SPEED_BASE`（50），基础速度必须满足
  `LINE_BASE_SPEED - 2×LINE_DIFF_STEP > CAR_SPEED_BASE`，否则内侧轮掉进死区
  直接停转，变成急转而非修正。已加 `_Static_assert` 编译期拦截——取 55 会
  直接编译失败。
- **诊断输出拖慢控制周期**。每次图案变化 `printf` 一行约 2.6 ms（115200），
  占掉 10 ms 周期的四分之一，而跟线时图案变化很频繁。已加
  `LINE_DEBUG_PATTERN` 开关，**默认关**；pivot / lost / reacquire 这些低频
  事件仍然输出。
